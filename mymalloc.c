...
void print_implicit_free_list()
{
    ...
    while (index < MEM_SIZE)
    {
        ...
        unsigned short payload_size = chunk_size - sizeof(t_meta_data);

        printf("CHUNK #%d\n", number);
        printf("chunk_size : %hu\n", chunk_size);
        printf("payload_size : %hu\n", payload_size);
        printf("is free : %s\n", metadata_of_current_block->free == 1 ? "yes" : "no");
        printf("pointer of payload in memory: %p\n", current);
...
{
    // char* z = (char*)s_malloc(sizeof(char));
    // z = "selin";
    // printf("Here %c\n",z[3]);

    char *z = (char *)o_malloc(sizeof(char));
    ...
    printf("Size %lu and address: %p and value: %d\n", sizeof(z), z, *z);
}